...
class BankingSystem:
    def __init__(self, root):
        ...
        self.create_account_frame = tk.Frame(root, bg="#e0e0e0", bd=2, relief=tk.SUNKEN)
        self.create_account_frame.pack(pady=10, padx=10, fill=tk.X)

        self.acc_num_label = tk.Label(self.create_account_frame, text="Account Number:", bg="#e0e0e0")
        ...
        self.acc_holder_label = tk.Label(self.create_account_frame, text="Account Holder:", bg="#e0e0e0")
        ...
        self.initial_balance_label = tk.Label(self.create_account_frame, text="Initial Balance:", bg="#e0e0e0")
        ...
        self.create_acc_button = tk.Button(self.create_account_frame, text="Create Account", command=self.create_account, bg="#4caf50", fg="white")
        self.create_acc_button.grid(row=3, columnspan=2, pady=10)

        # Widgets for transactions
        self.transaction_frame = tk.Frame(root, bg="#e0e0e0", bd=2, relief=tk.SUNKEN)
        self.transaction_frame.pack(pady=10, padx=10, fill=tk.X)

        self.trans_acc_num_label = tk.Label(self.transaction_frame, text="Account Number:", bg="#e0e0e0")
        ...
        self.amount_label = tk.Label(self.transaction_frame, text="Amount:", bg="#e0e0e0")
        self.amount_label.grid(row=1, column=0, padx=5, pady=5)
        self.amount_entry = tk.Entry(self.transaction_frame)
        self.amount_entry.grid(row=1, column=1, padx=5, pady=5)

        self.deposit_button = tk.Button(self.transaction_frame, text="Deposit", command=self.deposit, bg="#2196f3", fg="white")
        self.deposit_button.grid(row=2, column=0, pady=10, padx=5)

        self.withdraw_button = tk.Button(self.transaction_frame, text="Withdraw", command=self.withdraw, bg="#f44336", fg="white")
        self.withdraw_button.grid(row=2, column=1, pady=10, padx=5)

        # Widgets for account information
        self.info_frame = tk.Frame(root, bg="#e0e0e0", bd=2, relief=tk.SUNKEN)
        self.info_frame.pack(pady=10, padx=10, fill=tk.X)

        self.info_acc_num_label = tk.Label(self.info_frame, text="Account Number:", bg="#e0e0e0")
        ...
        self.info_button = tk.Button(self.info_frame, text="Display Info", command=self.display_info, bg="#9c27b0", fg="white")
